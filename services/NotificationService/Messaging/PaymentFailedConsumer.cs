...
public class PaymentFailedConsumer : BackgroundService
{
    ...
    private const string Queue = "payment.failed";
        ...
        consumer.Received += async (_, ea) =>
        {
            ...
            try
            {
                ...
                await sender.SendAsync(
                    ...
                    message: $"❌ Payment failed for Order #{payload.OrderId}. Reason: CONTROLLED_TEST_FAILURE. Your stock has been released.");

                _channel!.BasicAck(ea.DeliveryTag, false);
                _logger.LogInformation("[Notification] PaymentFailed sent for OrderId={OrderId}", payload.OrderId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[Notification] Error processing PaymentFailed");
